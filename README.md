...
==== GCRM I/O kernel in C ====

This software is the I/O kernel of Global Cloud Resolving Model (GCRM)
simulation codes. GCRM was developed at the Colorado State University (URL:
http://kiwi.atmos.colostate.edu/gcrm) and its I/O module, called GIO library,
was developed at Pacific Northwest National Laboratory (URL:
https://svn.pnl.gov/gcrm). GCRM and GIO are programmed in Fortran90.

This software extracts the I/O kernel from GCRM and converts it into C for more
flexible parameter settings (e.g. number of MPI processes, max level) and
...
---- Input parameters ----
 ...
 == parameters determine the number of solution variables ==
    ...
    parameter "io_config_file" points to a file describing the I/O
    configuration.
...
---- Number of processes to run and command-line arguments ----
    ...
    is dividible by the number of MPI processes.
        int nblocks[10] = {10, 40, 160, 640, 2560, 10240, 40960, 163840,
                           655360, 2621440};
    For example, if nprocs = 80, then sbdmn_iota will be 2, because
    nblocks[2] is 160 which is divisible by nprocs.
